...
describe('createExtensionRegistry — register()', () => {
  ...
  it('getStatus() retourne ready après register()', () => {
    const registry = createExtensionRegistry()
    registry.register(makeRenderer('markdown', ['.md']))
    expect(registry.getStatus('markdown')).toBe('ready')
  })

  it('émet ow:extension:ready après register()', () => {
    dispatchedEvents.length = 0
    const registry = createExtensionRegistry()
    registry.register(makeRenderer('myext', ['.txt']))
...
describe('createExtensionRegistry — load()', () => {
  ...
  it('getStatus() retourne undefined pour une extension inconnue', () => {
    const registry = createExtensionRegistry()
