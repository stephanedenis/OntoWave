...
test('Validation des liens externes sans hash', async ({ page }) => {
  ...
  // Vérifier le lien GitHub dans la section licence française
  ...
  await test.step('Vérifier le lien GitHub anglais', async () => {
    ...
    await page.waitForSelector('#lang-en', { state: 'visible' });
    
    // Chercher le lien "GitHub repository"
